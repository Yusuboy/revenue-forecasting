...
class RevenueForecastSarimax:
    ...
    def __init__(
        self,
        exoc_variables_fin=["D/M FIN"],
        exoc_variables_ind=["D/M IND"],
        exoc_variables_ns=["D/M NS"],
    ):
    ...
    def train_model(self, train_start, train_end):
        ...
        seasonal_order = (
            ...
            1,
            12,
        )  # (P, D, Q, s) for seasonal component (annual seasonality)

        # SARIMAX model for Revenue FIN
        model_FIN = sm.tsa.statespace.SARIMAX(
            self.train_data["Revenue FIN"],
            exog=self.train_data[self.exoc_variables_fin],
            order=order,
            seasonal_order=seasonal_order,
            trend="ct",
        )  # 'ct' includes both constant and linear trend
        self.models["FIN"] = model_FIN.fit()

        # SARIMAX model for Revenue IND
        model_IND = sm.tsa.statespace.SARIMAX(
            self.train_data["Revenue IND"],
        ...
        model_NS = sm.tsa.statespace.SARIMAX(
            self.train_data["Revenue NS"],
    ...
    def forecast(self, forecast_start, forecast_end):
        ...
        forecast_fin = self.models["FIN"].predict(
            start=forecast_data.index[0], end=forecast_data.index[-1], exog=exog_fin
        )
        forecast_ind = self.models["IND"].predict(
            start=forecast_data.index[0], end=forecast_data.index[-1], exog=exog_ind
        )
        forecast_ns = self.models["NS"].predict(
            start=forecast_data.index[0], end=forecast_data.index[-1], exog=exog_ns
        )
...
if __name__ == "__main__":
    ...
    # Validating
    service.validate(
        forecast_fin, forecast_ind, forecast_ns, "2024-04-01", "2024-09-30"
    )
