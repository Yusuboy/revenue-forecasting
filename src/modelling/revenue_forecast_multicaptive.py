...
class RevenueForecastMulticaptive:
    ...
    def __init__(self):
        ...
        self.data_models = self.data[['Month_index', 'Revenue FIN', 'Revenue IND', 'Revenue NS']]
    ...
    def train_model(self, train_start, train_end):
        ...
        # Create a DataFrame with the 'Month' column and your data
        #self.monthly_multi_FIN = pd.DataFrame({'Month': months, 'Month_number' : month_numbers, 'Values': self.multiplicative_model_FIN.seasonal[3:15]})
        self.monthly_multi_FIN = pd.DataFrame({'Month': months, 'Values': self.multiplicative_model_FIN.seasonal[3:15]})
        self.monthly_multi_IND = pd.DataFrame({'Month': months, 'Values': self.multiplicative_model_IND.seasonal[3:15]})
        self.monthly_multi_NS = pd.DataFrame({'Month': months, 'Values': self.multiplicative_model_NS.seasonal[3:15]})
