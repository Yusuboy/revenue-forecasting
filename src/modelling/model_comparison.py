...
from revenue_forecast_multicaptive_2 import RevenueForecastMulticaptive2
...
def model_comparison():
    ...
    multicaptive_model_normalized = RevenueForecastMulticaptive2()
...
def model_comparison_rolling_validation():
    ...
    for period in rolling_validation_periods:
        ...
        for model_name, model_results in zip(['SARIMAX', 'Runrate Notrend', 'Runrate Trend', 'Multicaptive', 'Multicaptive Normalized'], [results_sm, results_rn, results_rt, results_mc, results_mn]):
            csv_data.append({
                'Period': period[1],
                'Model': model_name,
    ...
    for i, period in enumerate(rolling_validation_periods):
        date = period[1]
        print(f'Month {date}')
        for model_name, model_results in zip(['SARIMAX', 'Runrate Notrend', 'Runrate Trend', 'Multicaptive', 'Multicaptive Normalized'], results[i]):
            print(f"  {model_name}:")
    ...
    for model_name, avg_errors in zip(['SARIMAX', 'Runrate Notrend', 'Runrate Trend', 'Multicaptive', 'Multicaptive Normalized'], [avg_errors_sm, avg_errors_rn, avg_errors_rt, avg_errors_mc, avg_errors_mn]):
        print(f"\n{model_name}:")
...
def forecast_12_months():
    ...
    forecast_fin_sm = forecast_fin_sm.tolist() if isinstance(forecast_fin_sm, pd.Series) else forecast_fin_sm
    forecast_ind_sm = forecast_ind_sm.tolist() if isinstance(forecast_ind_sm, pd.Series) else forecast_ind_sm
    forecast_ns_sm = forecast_ns_sm.tolist() if isinstance(forecast_ns_sm, pd.Series) else forecast_ns_sm
    
    forecast_fin_rn = forecast_fin_rn.tolist() if isinstance(forecast_fin_rn, pd.Series) else forecast_fin_rn
    forecast_ind_rn = forecast_ind_rn.tolist() if isinstance(forecast_ind_rn, pd.Series) else forecast_ind_rn
    forecast_ns_rn = forecast_ns_rn.tolist() if isinstance(forecast_ns_rn, pd.Series) else forecast_ns_rn
    
    forecast_fin_rt = forecast_fin_rt.tolist() if isinstance(forecast_fin_rt, pd.Series) else forecast_fin_rt
    forecast_ind_rt = forecast_ind_rt.tolist() if isinstance(forecast_ind_rt, pd.Series) else forecast_ind_rt
    forecast_ns_rt = forecast_ns_rt.tolist() if isinstance(forecast_ns_rt, pd.Series) else forecast_ns_rt
    
    forecast_fin_mc = forecast_fin_mc.tolist() if isinstance(forecast_fin_mc, pd.Series) else forecast_fin_mc
    forecast_ind_mc = forecast_ind_mc.tolist() if isinstance(forecast_ind_mc, pd.Series) else forecast_ind_mc
    forecast_ns_mc = forecast_ns_mc.tolist() if isinstance(forecast_ns_mc, pd.Series) else forecast_ns_mc
    
    forecast_fin_mn = forecast_fin_mn.tolist() if isinstance(forecast_fin_mn, pd.Series) else forecast_fin_mn
    forecast_ind_mn = forecast_ind_mn.tolist() if isinstance(forecast_ind_mn, pd.Series) else forecast_ind_mn
    forecast_ns_mn = forecast_ns_mn.tolist() if isinstance(forecast_ns_mn, pd.Series) else forecast_ns_mn

    # Create a list of months for columns
    months = pd.date_range(start=forecast_start, end=forecast_end, freq='M').strftime('%Y-%m').tolist()

    # Prepare data for each model and location
    forecast_data = {
        'Model': ['SARIMAX']*3 + ['Runrate without Trend']*3 + ['Runrate with Trend']*3 + ['Multicaptive']*3 + ['Multicaptive Normalized']*3,
    ...
    for i, month in enumerate(months):
        forecast_data[month] = [
            forecast_fin_sm[i], forecast_ind_sm[i], forecast_ns_sm[i],
            forecast_fin_rn[i], forecast_ind_rn[i], forecast_ns_rn[i],
            forecast_fin_rt[i], forecast_ind_rt[i], forecast_ns_rt[i],
            forecast_fin_mc[i], forecast_ind_mc[i], forecast_ns_mc[i],
            forecast_fin_mn[i], forecast_ind_mn[i], forecast_ns_mn[i]
        ]
