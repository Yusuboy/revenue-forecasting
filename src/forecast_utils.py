...
class ForecastUtils:
    ...
    def load_rate_normalized_data():
        ...
        for i, row in df.iterrows():
            revenue_fin = (
                multiplier_fin / row["raise_multiplier_fin"] * row["Revenue FIN"]
            )
            revenue_ind = (
                multiplier_fin / row["raise_multiplier_ind"] * row["Revenue IND"]
            )
    ...
    @staticmethod
    def split_data_train_validation_forecast(
        df,
        train_start,
        train_end,
        validatation_start,
        ...
        validation_data = df[
            (df.index >= validatation_start) & (df.index <= validatation_end)
        ]
    ...
    # This method expects that there is a date index created in the dataset. If using data load methods of this utility class, it's already done there.
    @staticmethod
    def split_data_train_validation(
        df, train_start, train_end, validatation_start, validatation_end
    ):
    ...
    def validation_results(
        ...
        actual_total = (
            validation_data["Revenue FIN"]
            + validation_data["Revenue IND"]
            + validation_data["Revenue NS"]
        )
        ...
        monthly_errors = pd.DataFrame(
            {
                ...
                "Forecast Total": pd.Series(
                    forecast_total, index=validation_data.index
                ),
            }
        )
        ...
        monthly_errors["Error% FIN"] = monthly_errors.apply(
            lambda row: (
                None
                if row["Actual FIN"] is None
                else (
                    (float(row["Actual FIN"]) - float(row["Forecast FIN"]))
                    / float(row["Actual FIN"])
                    * 100
                )
            ),
            axis=1,
        ...
        monthly_errors["Error% IND"] = monthly_errors.apply(
            lambda row: (
                None
                if row["Actual IND"] is None
                else (
                    (float(row["Actual IND"]) - float(row["Forecast IND"]))
                    / float(row["Actual IND"])
                    * 100
                )
            ),
            axis=1,
        ...
        monthly_errors["Error% NS"] = monthly_errors.apply(
            lambda row: (
                None
                if row["Actual NS"] is None
                else (
                    (float(row["Actual NS"]) - float(row["Forecast NS"]))
                    / float(row["Actual NS"])
                    * 100
                )
            ),
        ...
        monthly_errors["Error% Total"] = monthly_errors.apply(
            lambda row: (
                None
                if row["Actual Total"] is None
                else (
                    (float(row["Actual Total"]) - float(row["Forecast Total"]))
                    / float(row["Actual Total"])
                    * 100
                )
            ),
            axis=1,
        ...
        if plot_errors:
            ForecastUtils.plot_results(
                train_data, validation_data, forecast_FIN, forecast_IND, forecast_NS
            )
        ...
        print(
            f"Mean of total error absolute values in the validation period: {comparation_values[0]:,.1f}%"
        )
        print(
            f"Mean of FIN error absolute values in the validation period: {comparation_values[1]:,.1f}%"
        )
        print(
            f"Mean of IND error absolute values in the validation period: {comparation_values[2]:,.1f}%"
        )
        print(
            f"Mean of NS error absolute values in the validation period: {comparation_values[3]:,.1f}%"
        )
