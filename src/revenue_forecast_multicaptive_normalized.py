...
class RevenueForecastMulticaptiveNormalized:
    ...
    def __init__(self):
        ...
        self.data_models = self.data[
            ["Month_index", "D/M FIN", "Revenue FIN", "Revenue IND", "Revenue NS"]
        ]
    ...
    def train_model(self, train_start, train_end):
        ...
        training_data = ForecastUtils.split_data(
            self.data_models, train_start, train_end
        )
        ...
        training_data["Revenue_FIN_normalized"] = (
            training_data["Revenue FIN"] / training_data["Working_days_FIN"]
        )
        training_data["Revenue_IND_normalized"] = (
            training_data["Revenue IND"] / training_data["Working_days_IND"]
        )
        training_data["Revenue_NS_normalized"] = (
            training_data["Revenue NS"] / training_data["Working_days_NS"]
        )
        ...
        data_FIN = training_data[["Month_index", "Revenue_FIN_normalized"]].set_index(
            "Month_index"
        )
        data_IND = training_data[["Month_index", "Revenue_IND_normalized"]].set_index(
            "Month_index"
        )
        data_NS = training_data[["Month_index", "Revenue_NS_normalized"]].set_index(
            "Month_index"
        )
        ...
        self.multiplicative_model_FIN = seasonal_decompose(
            data_FIN["Revenue_FIN_normalized"], model="multiplicative", period=12
        )
        self.multiplicative_model_IND = seasonal_decompose(
            data_IND["Revenue_IND_normalized"], model="multiplicative", period=12
        )
        self.multiplicative_model_NS = seasonal_decompose(
            data_NS["Revenue_NS_normalized"], model="multiplicative", period=12
        )
        ...
        self.monthly_multi_FIN = pd.DataFrame(
            {
                "Month": months,
                "Values": self.multiplicative_model_FIN.seasonal[
                    first_january_row : first_january_row + 12
                ],
            }
        )
        self.monthly_multi_IND = pd.DataFrame(
            {
                "Month": months,
                "Values": self.multiplicative_model_IND.seasonal[
                    first_january_row : first_january_row + 12
                ],
            }
        )
        self.monthly_multi_NS = pd.DataFrame(
            {
                "Month": months,
                "Values": self.multiplicative_model_NS.seasonal[
                    first_january_row : first_january_row + 12
                ],
            }
        )
    ...
    def forecast(self, forecast_start, forecast_end):
        ...
        # print(forecast_data)
        slope_fin, intercept_fin, r_value_fin, p_value_fin, std_err_fin = linregress(
            self.cleaned_trend_FIN.index, self.cleaned_trend_FIN.values
        )
        slope_ind, intercept_ind, r_value_ind, p_value_ind, std_err_ind = linregress(
            self.cleaned_trend_IND.index, self.cleaned_trend_IND.values
        )
        slope_ns, intercept_ns, r_value_ns, p_value_ns, std_err_ns = linregress(
            self.cleaned_trend_NS.index, self.cleaned_trend_NS.values
        )
        ...
        predictions_df_FIN = pd.DataFrame(
            {
                ...
                "Predictions": [
                    (slope_fin * x + intercept_fin)
                    * multis_per_predicted_month_fin.values[i][1]
        ...
        predictions_df_IND = pd.DataFrame(
            {
                "Month_index": months_to_predict,
                "Predictions": [
                    (slope_ind * x + intercept_ind)
                    * multis_per_predicted_month_ind.values[i][1]
                    for i, x in enumerate(months_to_predict)
                ],
            }
        )
        predictions_df_NS = pd.DataFrame(
            {
                "Month_index": months_to_predict,
                "Predictions": [
                    (slope_ns * x + intercept_ns)
                    * multis_per_predicted_month_ns.values[i][1]
                    for i, x in enumerate(months_to_predict)
                ],
            }
        )
    ...
    def validate(
        ...
        result = ForecastUtils.validation_results(
            ...
            model_name="multicaptive_normalized",
...
if __name__ == "__main__":
    ...
    service = RevenueForecastMulticaptiveNormalized()

    # Train the model (it's internal submodels)
    service.train_model("2021-10-01", "2024-09-30")

    # Forecasting
    forecast_fin, forecast_ind, forecast_ns, forecast_total = service.forecast(
