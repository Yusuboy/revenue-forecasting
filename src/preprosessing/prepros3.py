...
input_file_path = '2024-09-23 Liikevaihto_data.csv'
output_file_path = 'Revenue data - preprosessed.csv'
report_file_path = 'Revenue data summary report.csv'
location_revenue_file_path = 'Revenue data by location.csv'
...
def calculate_sum_per_month(df):
    ...
    df['Location2'] = df['Location2'].map(location2_map)
    ...
    # Group by Year, Month, and Location to calculate the sum per month and per location
    monthly_sum_per_location_2 = df.groupby(['Year', 'Month', 'Location2'])['Resource Amount'].sum().reset_index()

    # Rename columns for clarity
    monthly_sum_per_location.columns = ['Year', 'Month', 'Location', 'Total Resource Amount per Location']
    monthly_sum_per_location_2.columns = ['Year', 'Month', 'Location2', 'Revenue per Location2']

    # Optionally return the result if further processing is needed
    return monthly_sum, monthly_sum_per_location, monthly_sum_per_location_2
...
def print_report(monthly_sums, monthly_sums_per_location, monthly_avg_rates, monthly_avg_rates_per_location, total_hours_per_month, hours_per_location_monthly):
    ...
    # ----------------- New Report based on 'Location2' - added more granularity to locations -----------------
    # Pivot the data based on 'Location2'
    pivot_sums_per_location_2 = monthly_sum_per_location_2.pivot_table(
        index=['Year', 'Month'], columns='Location2', values='Revenue per Location2'
    ).reset_index()

    # Add a column that sums the values from all refined locations
    pivot_sums_per_location_2['Total'] = pivot_sums_per_location_2.iloc[:, 2:].sum(axis=1)

    # Rename the columns for clarity
    pivot_sums_per_location_2.columns = [f'Revenue {col}' if col not in ['Year', 'Month'] else col for col in pivot_sums_per_location_2.columns]

    # Save the new report to a CSV file
    pivot_sums_per_location_2.to_csv(location_revenue_file_path, index=False)
...
monthly_sums, monthly_sums_per_location, monthly_sum_per_location_2 = calculate_sum_per_month(df)
