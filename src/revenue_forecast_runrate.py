...
class RevenueForecastRunrate:
    ...
    def train_model(self, train_start, train_end):
        ...
        first_year = (
            self.train_data.iloc[0]["Year"]
            if (self.train_data.iloc[0]["Month"] == 1)
            else self.train_data.iloc[0]["Year"] + 1
        )
        last_year = (
            self.train_data.iloc[-1]["Year"]
            if (self.train_data.iloc[-1]["Month"] == 12)
            else self.train_data.iloc[-1]["Year"] - 1
        )
        cyclic_years = self.train_data[
            (self.train_data["Year"] >= first_year)
            & (self.train_data["Year"] <= last_year)
        ]

        # Calculate cyclicity for each revenue category, taking into account the working days
        self.cyclicity_FIN = cyclic_years.groupby("Month").apply(
            lambda x: (x["Revenue FIN"] / x["D/M FIN"]).mean()
        )
        self.cyclicity_IND = cyclic_years.groupby("Month").apply(
            lambda x: (x["Revenue IND"] / x["D/M IND"]).mean()
        )
        self.cyclicity_NS = cyclic_years.groupby("Month").apply(
            lambda x: (x["Revenue NS"] / x["D/M NS"]).mean()
        )

        # Normalize the revenues in train data
        self.train_data.loc[:, "Normalized Revenue FIN"] = self.train_data[
            "Revenue FIN"
        ] / (
            self.train_data["Month"].map(self.cyclicity_FIN)
            * self.train_data["D/M FIN"]
        )
        self.train_data.loc[:, "Normalized Revenue IND"] = self.train_data[
            "Revenue IND"
        ] / (
            self.train_data["Month"].map(self.cyclicity_IND)
            * self.train_data["D/M IND"]
        )
        self.train_data.loc[:, "Normalized Revenue NS"] = self.train_data[
            "Revenue NS"
        ] / (
            self.train_data["Month"].map(self.cyclicity_NS) * self.train_data["D/M NS"]
        )
        ...
        if self.use_trend:
            self.trend_FIN = calculate_linear_trend(
                self.train_data["Normalized Revenue FIN"]
            )
            self.trend_IND = calculate_linear_trend(
                self.train_data["Normalized Revenue IND"]
            )
            self.trend_NS = calculate_linear_trend(
                self.train_data["Normalized Revenue NS"]
            )
    ...
    def forecast(self, forecast_start, forecast_end):
        ...
        forecast_data = ForecastUtils.split_data(
            self.data, forecast_start, forecast_end
        )
        ...
        forecast_run_rate_FIN = weighted_run_rate(
            self.run_rate_period["Normalized Revenue FIN"]
        )
        forecast_run_rate_IND = weighted_run_rate(
            self.run_rate_period["Normalized Revenue IND"]
        )
        forecast_run_rate_NS = weighted_run_rate(
            self.run_rate_period["Normalized Revenue NS"]
        )
        ...
        if self.use_trend:
            forecast_FIN = [
                self.cyclicity_FIN[month]
                * (forecast_run_rate_FIN + self.trend_FIN * (i + 1))
                * forecast_data.loc[forecast_data["Month"] == month, "D/M FIN"].values[
                    0
                ]
                for i, month in enumerate(forecast_months)
            ]
            forecast_IND = [
                self.cyclicity_IND[month]
                * (forecast_run_rate_FIN + self.trend_IND * (i + 1))
                * forecast_data.loc[forecast_data["Month"] == month, "D/M IND"].values[
                    0
                ]
                for i, month in enumerate(forecast_months)
            ]
            forecast_NS = [
                self.cyclicity_NS[month]
                * (forecast_run_rate_FIN + self.trend_NS * (i + 1))
                * forecast_data.loc[forecast_data["Month"] == month, "D/M NS"].values[0]
                for i, month in enumerate(forecast_months)
            ]
        else:
            forecast_FIN = [
                self.cyclicity_FIN[month]
                * forecast_run_rate_FIN
                * forecast_data.loc[forecast_data["Month"] == month, "D/M FIN"].values[
                    0
                ]
    ...
    def validate(
        ...
        validation_data = ForecastUtils.split_data(
            self.data, validation_start, validation_end
        )
        model_name = (
            "runrate without trend" if not self.use_trend else "runrate with trend"
        )
        result = ForecastUtils.validation_results(
            validation_data,
            forecast_fin,
            forecast_ind,
            forecast_ns,
...
if __name__ == "__main__":
    ...
    # Forecasting
    forecast_fin, forecast_ind, forecast_ns, forecast_total = (
        service_without_trend.forecast("2024-11-01", "2024-12-31")
    )
    ...
    # Validating
    service_without_trend.validate(
        forecast_fin, forecast_ind, forecast_ns, "2024-11-01", "2024-12-31"
    )
